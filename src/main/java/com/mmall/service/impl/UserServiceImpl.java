...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> checkAnswer(String username,String question,String answer){
        ...
        if(resultCount>0){
            ...
            //将forgetToken放在本地的cache中，并且设置有效期
    ...
    public ServerResponse<String> resetPassword(String passwordOld,String passwordNew,User user){
        //防止横向越权，要校验一下这个用户的旧密码，一定要指定是这个用户的，因为我们会查询一个count（1）出来，如果不指定id,那么结果就是true了
        ...
        System.err.println("updatecout："+updateCount);
        if(updateCount>0){
    ...
    public ServerResponse<User> updateInformation(User user){
        //username是不能被更新的
        //email也要进行一个校验，校验新的email是不是已经存在，并且存在的email如果相同的话，不是我们当前的这个用户的.
